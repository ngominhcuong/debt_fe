...
function AgingPanel({ aging }: Readonly<{ aging: ArDebtAgingResult }>) {
  return (
        ...
        {(
          [
            ...
            [
              "Nợ khó đòi",
              aging.hardToCollect,
              aging.hardToCollect > 0 ? "text-amber-600" : "",
            ],
            [
              "Nợ không thể đòi",
              aging.impossible,
              aging.impossible > 0 ? "text-destructive" : "",
            ],
...
// ─── Invoice detail table (Chi tiết) ────────────────────────────────────────

function InvoiceDetailTable({
  invoices,
}: Readonly<{ invoices: DebtInvoiceRow[] }>) {
      ...
      >
        ...
        <tfoot>
          <tr className="bg-muted font-bold border-t-2">
            <td colSpan={7} className="px-3 py-1.5 text-xs border-r">
              Tổng
            </td>
            <td className="px-3 py-1.5 text-right font-mono text-xs border-r">
              {fmtVND(totalGrand)}
...
function DetailPanel({
  ...
  token: string;
  onClose: () => void;
  onCollect: (row: CustomerDebtRow) => void;
  onRemind: (id: string) => void;
  reminding: string | null;
}>) {
  const { data, isLoading } = useQuery({
    queryKey: ["ar-debt-detail", row.customerId, token],
    queryFn: () => api.arDebt.detail(row.customerId, token).then((r) => r.data),
    enabled: !!token,
    staleTime: 30_000,
  });
      ...
      <div className="flex items-center justify-between px-6 py-2 border-b bg-muted/30 flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5 shrink-0 ml-4">
          <Button
            ...
            onClick={() => onRemind(row.customerId)}
          >
...
export default function ARDebtsPage() {
  ...
  const [selectedRow, setSelectedRow] = useState<CustomerDebtRow | null>(null);
  ...
  // Keep selectedRow data fresh if it is still in the current page
  const selectedInView =
    rows.find((r) => r.customerId === selectedRow?.customerId) ?? null;
  const panelOpen = selectedInView !== null;

  async function handleRemind(customerId: string) {
    setReminding(customerId);
    try {
      await api.arDebt.remind(customerId, token);
      toast.success("Đã gửi email nhắc nợ thành công", { duration: 4000 });
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Lỗi khi gửi nhắc nợ");
    } finally {
      setReminding(null);
    }
  }
    ...
    <>
      {collectTarget && (
        <CollectPaymentDialog
          ...
          customerCode={collectTarget.code}
      ...
      >
            ...
            <div className="flex flex-wrap gap-2 items-end px-6 pt-4 pb-3 border-b flex-shrink-0">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Tìm khách hàng, MST..."
                  value={q}
                  onChange={(e) => {
                    setQ(e.target.value);
                    setPage(1);
                ...
                <tbody>
                  {(() => {
                    ...
                    return rows.map((row, i) => {
                      const isSelected =
                        selectedInView?.customerId === row.customerId;
                      let rowBg: string;
                      if (isSelected) {
                          ...
                          >
                            <div className="flex gap-1 justify-center">
                              <Button
                                ...
                                disabled={reminding === row.customerId}
                                onClick={() => handleRemind(row.customerId)}
                              >
                                {reminding === row.customerId ? (
        ...
        {/* ── Bottom panel: only when a row is selected ── */}
        {selectedInView && (
          <>
