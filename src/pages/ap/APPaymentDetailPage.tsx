...
export default function APPaymentDetailPage() {
  ...
  const {
    ...
    queryKey: ["payment-detail", id, token],
    queryFn: () => {
      if (!id) throw new Error("Missing payment id");
      return api.payment.getById(id, token);
    },
    enabled: !!id && !!token,
    staleTime: 60_000,
  });
  ...
  return (
    <div className="flex flex-col gap-4 p-6">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 ml-2">
            <VoucherStatusBadge isPosted={payment.lines.length > 0} />
          </div>
        </div>

        <div className="flex gap-2">
          <Button size="sm" onClick={() => navigate("/ap/payments")}>
            Danh sách
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-4">
        {/* Left — supplier & transaction detail */}
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4">
              <InfoRow label="Mã nhà cung cấp" value={payment.supplier.code} />
              <InfoRow label="Tên nhà cung cấp" value={payment.supplier.name} />
              <InfoRow
                label="Mã số thuế / CCCD"
                value={payment.supplier.taxCode ?? "—"}
              />
              <InfoRow
                label="Địa chỉ"
                value={payment.supplier.address ?? "—"}
              />
              <InfoRow label="Người nhận" value={payment.recipient ?? "—"} />
              <InfoRow label="Lý do" value={payment.reason ?? "—"} />
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4">
              <InfoRow
                ...
                value={fmtDate(payment.accountingDate)}
              />
              <InfoRow
                label="Ngày phiếu chi"
                value={fmtDate(payment.paymentDate)}
              />
              <InfoRow label="Số chứng từ" value={payment.paymentNumber} bold />
            </div>
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-3">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Tổng tiền chi</span>
